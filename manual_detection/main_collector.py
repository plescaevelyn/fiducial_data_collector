...
class FiducialDataCollector:
    ...
    def _run_single_test(self, config: TestConfiguration) -> bool:
        ...
        # Use attributes from the config dataclass/object safely
        print(f"Description: {getattr(config, 'description', '')}")
        # some configurations may use 'test_type' or 'type'
        print(f"Type: {getattr(config, 'test_type', getattr(config, 'type', ''))}")
        print(f"Marker type: {getattr(config, 'marker_type', '')}")
        mid = getattr(config, 'marker_id', None)
        if mid is not None:
            print(f"Marker ID: {mid}")
        param = getattr(config, 'param', None)
        if param is not None:
            print(f"Parameter: {param}")
        est = getattr(config, 'estimated_seconds', 0)
        print(f"Estimated duration: {est}s")

        
        print("\nINSTRUCTIONS:")
        print(f"   {getattr(config, 'special_instructions', '')}")
